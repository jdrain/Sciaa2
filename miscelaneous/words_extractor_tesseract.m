...
%values. This program is for extracting words which are sutable to run
%throgh the tesseract, its just a method to check if the accuracy of ouptut is
%good if i pass the words.
...
for iterate=3:length(subfolder_names)
        ...
        mkdir('C:\Users\akhil\Documents\MATLAB\Allfiles\july-cleaning\smaller_output_original_files\printed\Individual_Words\',subfolder_names(iterate).name);
        for i = 1 : length(srcFiles)
            ...
            for object = 1:length(stats)
                ...
                path = strcat('C:\Users\akhil\Documents\MATLAB\Allfiles\july-cleaning\smaller_output_original_files\printed\Individual_Words\',subfolder_names(iterate).name);
                path = strcat(path,'\');
